...
//disarmcode(0xe755553e);
			...
			*/
			...
			//psg noise test: printf("pitch2: %d \n",(*(buffer_input+0))&0xffff);temp5--;
		...
		//disthumbcode(0x603a); //str r2,[r7,#0]
			...
			*/
			...
			printf("\n gbaSystemGlobal.GBADISPCNT(%x) DISPSTAT(%x) gbaSystemGlobal.GBAVCOUNT(%x)",(unsigned int)gba.GBADISPCNT,(unsigned int)gba.GBADISPSTAT,(unsigned int)gba.GBAVCOUNT);
			printf("\n GBAIE(%lu) GBAIF(%x) GBAIME(%x) EMULATING:(%x)",(unsigned int)gba.GBAIE,(unsigned int)gba.GBAIF,(unsigned int)gba.GBAIME,(unsigned int)gba.emulating);
