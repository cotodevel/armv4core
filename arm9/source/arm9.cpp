...
else iprintf("GBAROM open ERROR");


if (sectorsize_int32units!=copygbablock(0,0)){ //prepare disk_buf32 with rom data
	iprintf("failed filling disk_buf32(%d bytes) with rom offset 0",(int)(sectorsize_u32units+1));
	while(1);
}
//else
//	iprintf("success creating disk_buf32(%d bytes)",sectorsize_u32units);
	
if (sectorsize_int16units!=copygbablock(0,1)){ //prepare disk_buf16 with rom data
	iprintf("failed filling disk_buf16(%d bytes) with rom offset 0",(int)(sectorsize_u16units));
	while(1);
}
//else
//	iprintf("success creating disk_buf16(%d bytes)",sectorsize_u16units);

if (sectorsize_int8units!=copygbablock(0,2)){ //prepare disk_buf16 with rom data
	iprintf("failed filling disk_buf8(%d bytes) with rom offset 0",(int)(sectorsize_u8units));
	while(1);
}
...
/*
for(i=0;i<16;i++){
	iprintf("|(%d)[%x]| ",(int)i,(unsigned int)disk_buf32[i]);
}
while(1);
*/
...
nopinlasm();
	...
	//iprintf("gba wramstart:%x | gba wramend:%x |ewram start:%x | ewram end:%x",
	//(unsigned int)gbawram_start,(unsigned int)(gbawram_start+gbawram_size),(unsigned int)ewram_start,(unsigned int)(ewram_size));
