...
struct GBASystem{
    ...
    reg_pair reg[45];
	memoryMap map[256]; 	//field containing GBA mapped IO
    bool ioreadable[0x400]; //field of accessable IO memory
	...
	u8 rtc_datetime[0x7]; //2 -  2 | 7 bytes, date & time (year,month,day,day_of_week,hh,mm,ss)
...
void initemu(struct GBASystem *gba_inst);
...
//Stacking methods that aren't really for GBA (instead for general operations)
