...
u32 bios_cpuhalt(){
//+r0
gbavirtreg_cpu[0x0]=gba.cpustate=gbavirtreg_cpu[0x0];
//=r0
return 0;
}
...
int number = gbavirtreg_cpu[0x0]; //+r0
int denom = gbavirtreg_cpu[0x1]; //+r1

if(denom != 0) {
	gbavirtreg_cpu[0x0]=number / denom;						//exRegs[0] = number / denom;
	gbavirtreg_cpu[0x1]=number % denom;						//exRegs[1] = number % denom;
	s32 temp =	(s32)gbavirtreg_cpu[0x0];					//s32 temp = (s32)exRegs[0];
	gbavirtreg_cpu[0x3] = temp < 0 ? (u32)-temp : (u32)temp;//exRegs[3] = temp < 0 ? (u32)-temp : (u32)temp;
}
...
u32 bios_divarm(){
	...
	//tmp = r0
	u32 temp = gbavirtreg_cpu[0x0];
	
	//r0=r1
	gbavirtreg_cpu[0x0]=gbavirtreg_cpu[0x1];
	
	//r1=r0
	gbavirtreg_cpu[0x1]=temp;
...
u32 bios_sqrt(){
	...
	gbavirtreg_cpu[0x0] = (u16)sqrtasm((int)gbavirtreg_cpu[0x0]);//(u32)sqrt((double)dummyreg);
...
//+r0
s32 a =  -( ((s32)(gbavirtreg_cpu[0x0]*gbavirtreg_cpu[0x0])) >> 14);
...
b = ((b * a) >> 14) + 0x3651;
b = ((b * a) >> 14) + 0xA2F9;
a = ((s32)gbavirtreg_cpu[0x0] * b) >> 16;
//=r0
gbavirtreg_cpu[0x0] = a;
...
u32 bios_arctan2(){
//+r0
s32 x = gbavirtreg_cpu[0x0];
//+r1
s32 y = gbavirtreg_cpu[0x1];
u32 res = 0;
if (y == 0) {
	res = ((x>>16) & 0x8000);
} 
	...
	if (x == 0) {
		...
		if ((abs(x) > abs(y)) || ((abs(x) == abs(y)) && (!((x<0) && (y<0))))) {
			gbavirtreg_cpu[0x1] = x;
			gbavirtreg_cpu[0x0] = y << 14;
			bios_div();
			bios_arctan();
		if (x < 0)
			res = 0x8000 + gbavirtreg_cpu[0x0];
		else
			res = (((y>>16) & 0x8000)<<1) + gbavirtreg_cpu[0x0];
		} 
		else {
			gbavirtreg_cpu[0x0] = x << 14;
			bios_div();
			bios_arctan();
			res = (0x4000 + ((y>>16) & 0x8000)) - gbavirtreg_cpu[0x0];
      }
	}
}
gbavirtreg_cpu[0x0] = res;
return 0;
}
...
u32 source = gbavirtreg_cpu[0x0]; //+r0
u32 dest = gbavirtreg_cpu[0x1]; //+r1
u32 cnt = gbavirtreg_cpu[0x2]; //+r2
...
//r0
u32 source = gbavirtreg_cpu[0];
//r1
u32 dest = gbavirtreg_cpu[1];
//r2
u32 cnt = gbavirtreg_cpu[2];
...
u32 src = gbavirtreg_cpu[0x0];
u32 dest = gbavirtreg_cpu[0x1];
int num = gbavirtreg_cpu[0x2];
...
//#endif
	...
	u32 src = gbavirtreg_cpu[0x0]; 		//r0
	u32 dest = gbavirtreg_cpu[0x1];		//r1
	int num = gbavirtreg_cpu[0x2];		//r2
	int offset = gbavirtreg_cpu[0x3]; 	//r3
...
u32 source=gbavirtreg_cpu[0x0]; 	//u32 source = exRegs[0];
u32 dest=gbavirtreg_cpu[0x1];		//u32 dest = exRegs[1];
u32 header=gbavirtreg_cpu[0x2];	//u32 header = exRegs[2];
...
u32 source = gbavirtreg_cpu[0x0]; 	//exRegs[0];
u32 dest = gbavirtreg_cpu[0x1];	//exRegs[1];
...
int freq = cpuread_word(gbavirtreg_cpu[0x0]+4);	//CPUReadMemory(exRegs[0]+4);
double tmp;
tmp = ((double)(180 - gbavirtreg_cpu[0x1])) - ((double)gbavirtreg_cpu[0x2] / 256.f);
tmp = pow((double)2.f, tmp / 12.f);
gbavirtreg_cpu[0x0] = (int)((double)freq / tmp); //save to r0
...
//swi 0x2a
u32 bios_snddriverjmptablecopy(struct GBASystem * gba_inst){
//#ifdef DEV_VERSION
...
for(i = 0; i < 0x24; i++) {
	cpuwrite_word(gbavirtreg_cpu[0x0], 0x9c);	//CPUWriteMemory(dummyreg, 0x9c);
	dummyreg += 4;
}
